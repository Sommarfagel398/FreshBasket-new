...
export default function Cart() {
  ...
  const applyPromo = () => {
    setPromoError('');
    const result = validatePromo(promoInput.toUpperCase(), cartTotal);
    if (result.valid) setAppliedPromo(result.promo);
            ...
            <div className="p-4 rounded-3 sticky-top" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid #e8e4de', top: '80px', zIndex: 1 }}>
              ...
              <div className="mb-4">
                ...
                <div className="input-group">
                  <input type="text" className="form-control" placeholder="Enter code" value={promoInput} onChange={e => setPromoInput(e.target.value.toUpperCase())}
                ...
                {appliedPromo && <small className="d-block mt-1" style={{ color: 'var(--status-success-text)' }}><i className="bi bi-check-circle me-1"></i>"{appliedPromo.
                code}" applied!</small>}
              </div>
              ...
              {discount > 0 && (
                <div className="d-flex justify-content-between mb-2 small">
                  <span style={{ color: 'var(--status-success-text)' }}>Discount ({
              ...
              {currentUser ? (
                <button onClick={() => navigate('/checkout', { state: { discount, appliedPromo, finalTotal } })} className="btn w-100 fw-bold py-3" style={{ backgroundColor: 'var(--zest)', color: 'var(--surface-card)', border: 'none', borderRadius: 0, letterSpacing: '0.5px' }}>
