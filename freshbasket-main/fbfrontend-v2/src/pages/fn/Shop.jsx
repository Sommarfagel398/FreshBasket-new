...
export default function Shop() {
  ...
  const filtered = useMemo(() => {
    ...
    return res;
  }, [products, category, sort, search, priceMax]);
              ...
              <div className="p-4 rounded-3" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid #e8e4de' }}>
                ...
                <div className="d-flex flex-wrap gap-2">
                  {['Organic', 'Seasonal', 'Premium', 'Popular', 'Superfood'].map(tag => (
                    <button key={tag} onClick={() => setSearch(tag)} className="btn btn-sm rounded-pill" style={{ backgroundColor: 'var(--surface-muted)', color: 'var(--moss)', fontSize: '0.72rem', border: 'none', padding: '4px 12px' }}>
                      {tag}
                    </button>
                  ))}
                </div>
