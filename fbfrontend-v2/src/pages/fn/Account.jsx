...
export default function Account() {
  ...
  const getTabFromPath = (pathname) => {
    ...
    if (pathname.includes('/account/favourites') || pathname.includes('/account/favorites')) return 'favourites';
    return 'profile';
  };
  ...
  useEffect(() => {
    setForm({ name: currentUser?.name || '', phone: currentUser?.phone || '', address: currentUser?.address || '' });
  }, [currentUser]);
          ...
          <div className="col-12 col-md-3">
            ...
            <div className="p-2 rounded-3" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid #e8e4de' }}>
              {[['profile', 'bi-person', 'My Profile'], ['orders', 'bi-bag-check', 'My Orders'], ['favourites', 'bi-heart', 'Favourites']].map(([t, icon, label]) => (
                <button key={t} onClick={() => navigate(`/account${t === 'profile' ? '' : `/${t}`}`)} className="btn btn-sm d-flex align-items-center gap-2 w-100 text-start mb-1 px-3 py-2 rounded-2"
